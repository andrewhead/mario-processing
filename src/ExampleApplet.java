...
public class ExampleApplet extends PApplet {
    ...
    class MainBackgroundLayer extends LevelLayer {
        ...
        MainBackgroundLayer(Level owner) {
            super(owner, owner.width, owner.height, 0, 0, 0.75f, 0.75f);
    ...
    class MainLevelLayer extends LevelLayer {
        ...
        void addTriggers() {
            addTrigger(new KoopaTrigger(412, 0, 5, height, 350, height - 64, -0.2f, 0));
            addTrigger(new KoopaTrigger(562, 0, 5, height, 350, height - 64, -0.2f, 0));
            addTrigger(new KoopaTrigger(916, 0, 5, height, 350, height - 64, -0.2f, 0));
        }
...
//////////////
    ...
    class Koopa extends Interactor {
        ...
        void setStates() {
            ...
            walking.setAnimationSpeed(0.12f);
    ...
    abstract class Actor extends Positionable {
        ...
        void handleSpriteSwap(Sprite osprite, Sprite nsprite) {
            float ax1 = osprite.hanchor,
                    ...
                    ax2 = nsprite.hanchor,
                    ay2 = nsprite.vanchor;
            float dx = (ax2 - ax1) / 2.0f, dy = (ay2 - ay1) / 2.0f;
            x -= dx;
            y -= dy;
        }
...
// ====== ABSTRACT METHODS ======
    ...
    static class CollisionDetection {
        ...
        static float[] getLineRectIntersection(float[] line, float[] previous, float[] current) {
            ...
            if (debug)
                sketch.println(sketch.frameCount + ">    dotproduct result: start=" + inRangeSp + "/" + inRangeSc + ", end=" + inRangeEp + "/" + inRangeEc + ", sided=" + abovePrevious + "/" + aboveCurrent);
            ...
            if (inRangeForStart || inRangeForEnd) {
                if (debug)
                    sketch.println(sketch.frameCount + ">   this boundary is not involved in collisions for this frame (out of range).");
                return null;
            }
            ...
            if (abovePrevious == 4 && aboveCurrent == 4) {
                if (debug)
                    sketch.println(sketch.frameCount + ">   this box is not involved in collisions for this frame (inherently safe 'above' locations).");
                return null;
            } else if (0 < abovePrevious && abovePrevious < 4) {
                if (debug)
                    sketch.println(sketch.frameCount + ">   this box is not involved in collisions for this frame (never fully went through boundary).");
                return null;
            }
                ...
                for (i = 0; i < 8; i += 2) {
                    for (j = 0; j < 8; j += 2) {
                        ...
                        if (intersection != null) {
                            found = true;
                            if (debug)
                                sketch.println(sketch.frameCount + ">     collision detected on a box edge (box overlap).");
                        }
                    }
                }
            }
...
// global screens container
    ...
    class Pickup extends Actor {
        ...
        void setupStates() {
            ...
            pickup.sprite.setAnimationSpeed(0.25f);
            addState(pickup);
        }
...
// -- pathing informmation
    ...
    static class SpritePathChunker {
        ...
        static final float[] Tvalues = {-0.0640568928626056299791002857091370970011f,
                0.0640568928626056299791002857091370970011f,
                -0.1911188674736163106704367464772076345980f,
                0.1911188674736163106704367464772076345980f,
                -0.3150426796961633968408023065421730279922f,
                0.3150426796961633968408023065421730279922f,
                -0.4337935076260451272567308933503227308393f,
                0.4337935076260451272567308933503227308393f,
                -0.5454214713888395626995020393223967403173f,
                0.5454214713888395626995020393223967403173f,
                -0.6480936519369755455244330732966773211956f,
                0.6480936519369755455244330732966773211956f,
                -0.7401241915785543579175964623573236167431f,
                0.7401241915785543579175964623573236167431f,
                -0.8200019859739029470802051946520805358887f,
                0.8200019859739029470802051946520805358887f,
                -0.8864155270044010714869386902137193828821f,
                0.8864155270044010714869386902137193828821f,
                -0.9382745520027327978951348086411599069834f,
                0.9382745520027327978951348086411599069834f,
                -0.9747285559713094738043537290650419890881f,
                0.9747285559713094738043537290650419890881f,
                -0.9951872199970213106468008845695294439793f,
                0.9951872199970213106468008845695294439793f};

        // Legendre-Gauss weights for n=25
        static final float[] Cvalues = {0.1279381953467521593204025975865079089999f,
                0.1279381953467521593204025975865079089999f,
                0.1258374563468283025002847352880053222179f,
                0.1258374563468283025002847352880053222179f,
                0.1216704729278033914052770114722079597414f,
                0.1216704729278033914052770114722079597414f,
                0.1155056680537255991980671865348995197564f,
                0.1155056680537255991980671865348995197564f,
                0.1074442701159656343712356374453520402312f,
                0.1074442701159656343712356374453520402312f,
                0.0976186521041138843823858906034729443491f,
                0.0976186521041138843823858906034729443491f,
                0.0861901615319532743431096832864568568766f,
                0.0861901615319532743431096832864568568766f,
                0.0733464814110802998392557583429152145982f,
                0.0733464814110802998392557583429152145982f,
                0.0592985849154367833380163688161701429635f,
                0.0592985849154367833380163688161701429635f,
                0.0442774388174198077483545432642131345347f,
                0.0442774388174198077483545432642131345347f,
                0.0285313886289336633705904233693217975087f,
                0.0285313886289336633705904233693217975087f,
                0.0123412297999872001830201639904771582223f,
                0.0123412297999872001830201639904771582223f};
        ...
        static float[] getTimeValues(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float frames) {
            float curvelen = computeCubicCurveLength(1.0f, x1, y1, x2, y2, x3, y3, x4, y4),
        ...
        private static float computeCubicCurveLength(float z, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
            ...
            float z2 = z / 2.0f;  // interval-correction
